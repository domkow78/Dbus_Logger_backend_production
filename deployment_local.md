# Deployment Local – Raspberry Pi (Native + systemd)

Instrukcja wdrożenia backendu Dbus Logger bezpośrednio w systemie Raspberry Pi OS z wykorzystaniem Python venv i systemd.
...
```python
DEFAULT_PORTS = {
    'linux': '/dev/serial0',   # ← symlink Pi OS → ttyAMA0 (stabilny)
    ...
}
```
...
### Krok 2 – aktualizuj zależności (jeśli zmienione)
...
```bash
sudo systemctl restart dbus-logger-backend.service
...
# Zatrzymaj konfliktujący proces lub zmień port w aplikacji
```
...
### Usługa systemd nie startuje

```bash
# Sprawdź logi błędów
journalctl -u dbus-logger-backend.service -n 50

# Sprawdź poprawność ścieżek w pliku .service
systemctl cat dbus-logger-backend.service

# Sprawdź uprawnienia do katalogu
ls -la /home/pi/Dbus_Logger_backend_production

# Test uruchomienia ręcznego
cd /home/pi/Dbus_Logger_backend_production
source venv/bin/activate
python start_backend.py
```
